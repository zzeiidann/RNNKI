...
mod attention;
mod training;
mod metrics; // <<— tambahkan modul metrics

use data_loader::DataLoader;
use tokenizer::Tokenizer;
use model::AttentionRNN;
use training::Trainer;
use metrics::{rouge_all, rouge_1_f1}; // <<— import fungsi ROUGE
...
fn main() -> Result<()> {
    println!("Document Summarization with Attention RNN");
    println!("=========================================\n");
    ...
    let mut tokenizer = Tokenizer::new(10000); // vocab size
    ...
    // Create model
    println!("Initializing Attention RNN model...");
    ...
    // Save model
    println!("\nSaving model...");
    trainer.save_model("attention_rnn_model.pt")?;
    println!("✓ Model saved to attention_rnn_model.pt");

    // Test summarization + ROUGE on a few validation samples
